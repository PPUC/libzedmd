...
void
ZeDMD::SetLogCallback(ZeDMD_LogCallback callback, const void* userData) {
...
void
ZeDMD::IgnoreDevice(const char* const ignore_device) {
    m_pZeDMDComm->IgnoreDevice(ignore_device);
}

void
ZeDMD::SetDevice(const char* const device) {
    m_pZeDMDComm->SetDevice(device);
}

void
ZeDMD::SetFrameSize(uint16_t width, uint16_t height) {
    m_romWidth = width;
    m_romHeight = height;
...
void
ZeDMD::SetRGBOrder(uint8_t rgbOrder) {
    m_pZeDMDComm->QueueCommand(ZEDMD_COMM_COMMAND::RGBOrder, rgbOrder);
}

void
ZeDMD::SetBrightness(uint8_t brightness) {
...
void
ZeDMD::EnablePreDownscaling() {
    m_downscaling = true;
}

void
ZeDMD::DisablePreDownscaling() {
    m_downscaling = false;
}
...
void
ZeDMD::SetWiFiPassword(const char* const password) {
...
bool
ZeDMD::OpenWiFi(const char* ip, int port) {
...
bool
ZeDMD::Open(uint16_t width, uint16_t height) {
    if (Open()) {
...
void
ZeDMD::SetPalette(uint8_t* pPalette) {
    SetPalette(pPalette, 64);
}

void
ZeDMD::SetPalette(uint8_t* pPalette, uint8_t numColors) {
    m_paletteChanged = false;

    uint8_t* pPaletteNumber;
    switch (numColors) {
        case 4: pPaletteNumber = m_palette4; break;
        case 16: pPaletteNumber = m_palette16; break;
        case 64: pPaletteNumber = m_palette64; break;
        default: return;
    }
...
void
ZeDMD::SetDefaultPalette(uint8_t bitDepth) {
    switch (bitDepth) {
        case 2: SetPalette(m_DmdDefaultPalette2Bit, 4); break;

        default: SetPalette(m_DmdDefaultPalette4Bit, 16);
    }
}

uint8_t*
ZeDMD::GetDefaultPalette(uint8_t bitDepth) {
    switch (bitDepth) {
        case 2: return m_DmdDefaultPalette2Bit; break;

        default: return m_DmdDefaultPalette4Bit;
    }
}

void
ZeDMD::EnforceStreaming() {
    m_streaming = true;
}
...
void
ZeDMD::RenderGray2(uint8_t* pFrame) {
...
void
ZeDMD::RenderGray4(uint8_t* pFrame) {
    if (!(m_usb || m_wifi) || !(UpdateFrameBuffer8(pFrame) || m_paletteChanged)) {
        return;
    }
...
void
ZeDMD::RenderColoredGray6(uint8_t* pFrame, uint8_t* pPalette, uint8_t* pRotations) {
    SetPalette(pPalette);
    RenderColoredGray6(pFrame, pRotations);
}

void
ZeDMD::RenderColoredGray6(uint8_t* pFrame, uint8_t* pRotations) {
    if (!(m_usb || m_wifi) || !(UpdateFrameBuffer8(pFrame) || m_paletteChanged)) {
        return;
    }
...
void
ZeDMD::RenderRgb24(uint8_t* pFrame) {
...
bool
ZeDMD::UpdateFrameBuffer8(uint8_t* pFrame) {
...
bool
ZeDMD::UpdateFrameBuffer24(uint8_t* pFrame) {
...
void
ZeDMD::Split(uint8_t* pPlanes, uint16_t width, uint16_t height, uint8_t bitlen, uint8_t* pFrame) {
...
void
ZeDMD::ConvertToRgb24(uint8_t* pFrameRgb24, uint8_t* pFrame, int size, uint8_t* pPalette) {
...
bool
ZeDMD::CmpColor(uint8_t* px1, uint8_t* px2, uint8_t colors) {
    if (colors == 3) {
...
void
ZeDMD::SetColor(uint8_t* px1, uint8_t* px2, uint8_t colors) {
    px1[0] = px2[0];
...
int
ZeDMD::Scale(uint8_t* pScaledFrame, uint8_t* pFrame, uint8_t colors, uint16_t* width, uint16_t* height) {
    int xoffset = 0;
    int yoffset = 0;
            ...
            for (int x = 0; x < m_romWidth; x += 2) {
                ...
                if (x < m_romWidth / 2) {
                    if (y < m_romHeight / 2) {
                        ...
                        } else if (CmpColor(&pFrame[upper_right], &pFrame[lower_left], colors)
                        ...
                        if (CmpColor(&pFrame[lower_left], &pFrame[lower_right], colors)
                            ...
                            SetColor(&pScaledFrame[target], &pFrame[lower_left], colors);
                        } else if (CmpColor(&pFrame[lower_right], &pFrame[upper_left], colors)
                    ...
                    if (y < m_romHeight / 2) {
                        if (CmpColor(&pFrame[upper_right], &pFrame[upper_left], colors)
                            ...
                            SetColor(&pScaledFrame[target], &pFrame[upper_right], colors);
                        } else if (CmpColor(&pFrame[upper_left], &pFrame[lower_right], colors)
                                   || CmpColor(&pFrame[upper_left], &pFrame[lower_left], colors)) {
                            SetColor(&pScaledFrame[target], &pFrame[upper_left], colors);
                        ...
                        if (CmpColor(&pFrame[lower_right], &pFrame[lower_left], colors)
                            ...
                            SetColor(&pScaledFrame[target], &pFrame[lower_right], colors);
                        } else if (CmpColor(&pFrame[lower_left], &pFrame[upper_right], colors)
                                   || CmpColor(&pFrame[lower_left], &pFrame[upper_left], colors)) {
                            SetColor(&pScaledFrame[target], &pFrame[lower_left], colors);
        ...
        for (int x = 0; x < m_romHeight; x++) {
            for (int y = 0; y < m_romWidth; y++) {
                ...
                if (!CmpColor(b, h, colors) && !CmpColor(d, f, colors)) {
                    SetColor(&pScaledFrame[(yoffset * frameWidth + x * 2 * frameWidth + y * 2 + xoffset) * colors],
                             CmpColor(d, b, colors) ? d : e, colors);
                    SetColor(&pScaledFrame[(yoffset * frameWidth + x * 2 * frameWidth + y * 2 + 1 + xoffset) * colors],
                             CmpColor(b, f, colors) ? f : e, colors);
                    SetColor(
                        &pScaledFrame[(yoffset * frameWidth + (x * 2 + 1) * frameWidth + y * 2 + xoffset) * colors],
                        CmpColor(d, h, colors) ? d : e, colors);
                    SetColor(
                        &pScaledFrame[(yoffset * frameWidth + (x * 2 + 1) * frameWidth + y * 2 + 1 + xoffset) * colors],
                        CmpColor(h, f, colors) ? f : e, colors);
                } else {
                    SetColor(&pScaledFrame[(yoffset * frameWidth + x * 2 * frameWidth + y * 2 + xoffset) * colors], e,
                             colors);
                    SetColor(&pScaledFrame[(yoffset * frameWidth + x * 2 * frameWidth + y * 2 + 1 + xoffset) * colors],
                             e, colors);
                    SetColor(
                        &pScaledFrame[(yoffset * frameWidth + (x * 2 + 1) * frameWidth + y * 2 + xoffset) * colors], e,
                        colors);
                    SetColor(
                        &pScaledFrame[(yoffset * frameWidth + (x * 2 + 1) * frameWidth + y * 2 + 1 + xoffset) * colors],
                        e, colors);
        ...
        for (int y = 0; y < m_romHeight; y++) {
            for (int x = 0; x < m_romWidth; x++) {
                for (uint8_t c = 0; c < colors; c++) {
                    pScaledFrame[((yoffset + y) * frameWidth + xoffset + x) * colors + c] =
                        pFrame[(y * m_romWidth + x) * colors + c];
                }
            }
        }
    }
...
ZEDMDAPI void
ZeDMD_IgnoreDevice(ZeDMD* pZeDMD, const char* const ignore_device) {
    return pZeDMD->IgnoreDevice(ignore_device);
}

ZEDMDAPI void
ZeDMD_SetDevice(ZeDMD* pZeDMD, const char* const device) {
    return pZeDMD->SetDevice(device);
}

ZEDMDAPI bool
ZeDMD_Open(ZeDMD* pZeDMD) {
    return pZeDMD->Open();
}

ZEDMDAPI bool
ZeDMD_OpenWiFi(ZeDMD* pZeDMD, const char* ip, int port) {
    return pZeDMD->OpenWiFi(ip, port);
}

ZEDMDAPI void
ZeDMD_Close(ZeDMD* pZeDMD) {
    return pZeDMD->Close();
}

ZEDMDAPI void
ZeDMD_SetFrameSize(ZeDMD* pZeDMD, uint16_t width, uint16_t height) {
    return pZeDMD->SetFrameSize(width, height);
}

ZEDMDAPI void
ZeDMD_SetPalette(ZeDMD* pZeDMD, uint8_t* pPalette, uint8_t numColors) {
    return pZeDMD->SetPalette(pPalette, numColors);
}

ZEDMDAPI void
ZeDMD_SetDefaultPalette(ZeDMD* pZeDMD, uint8_t bitDepth) {
    return pZeDMD->SetDefaultPalette(bitDepth);
}

ZEDMDAPI uint8_t*
ZeDMD_GetDefaultPalette(ZeDMD* pZeDMD, uint8_t bitDepth) {
    return pZeDMD->GetDefaultPalette(bitDepth);
}

ZEDMDAPI void
ZeDMD_LedTest(ZeDMD* pZeDMD) {
    return pZeDMD->LedTest();
}

ZEDMDAPI void
ZeDMD_EnableDebug(ZeDMD* pZeDMD) {
    return pZeDMD->EnableDebug();
}

ZEDMDAPI void
ZeDMD_DisableDebug(ZeDMD* pZeDMD) {
    return pZeDMD->DisableDebug();
}

ZEDMDAPI void
ZeDMD_SetRGBOrder(ZeDMD* pZeDMD, uint8_t rgbOrder) {
    return pZeDMD->SetRGBOrder(rgbOrder);
}

ZEDMDAPI void
ZeDMD_SetBrightness(ZeDMD* pZeDMD, uint8_t brightness) {
    return pZeDMD->SetBrightness(brightness);
}

ZEDMDAPI void
ZeDMD_SaveSettings(ZeDMD* pZeDMD) {
    return pZeDMD->SaveSettings();
}

ZEDMDAPI void
ZeDMD_EnablePreDownscaling(ZeDMD* pZeDMD) {
    return pZeDMD->EnablePreDownscaling();
}

ZEDMDAPI void
ZeDMD_DisablePreDownscaling(ZeDMD* pZeDMD) {
    return pZeDMD->DisablePreDownscaling();
}

ZEDMDAPI void
ZeDMD_EnablePreUpscaling(ZeDMD* pZeDMD) {
    return pZeDMD->EnablePreUpscaling();
}

ZEDMDAPI void
ZeDMD_DisablePreUpscaling(ZeDMD* pZeDMD) {
    return pZeDMD->DisablePreUpscaling();
}

ZEDMDAPI void
ZeDMD_EnableUpscaling(ZeDMD* pZeDMD) {
    return pZeDMD->EnableUpscaling();
}

ZEDMDAPI void
ZeDMD_DisableUpscaling(ZeDMD* pZeDMD) {
    return pZeDMD->DisableUpscaling();
}

ZEDMDAPI void
ZeDMD_SetWiFiSSID(ZeDMD* pZeDMD, const char* const ssid) {
    return pZeDMD->SetWiFiSSID(ssid);
}

ZEDMDAPI void
ZeDMD_SetWiFiPassword(ZeDMD* pZeDMD, const char* const password) {
    return pZeDMD->SetWiFiPassword(password);
}

ZEDMDAPI void
ZeDMD_SetWiFiPort(ZeDMD* pZeDMD, int port) {
    return pZeDMD->SetWiFiPort(port);
}

ZEDMDAPI void
ZeDMD_EnforceStreaming(ZeDMD* pZeDMD) {
    return pZeDMD->EnforceStreaming();
}

ZEDMDAPI void
ZeDMD_ClearScreen(ZeDMD* pZeDMD) {
    return pZeDMD->ClearScreen();
}

ZEDMDAPI void
ZeDMD_RenderGray2(ZeDMD* pZeDMD, uint8_t* frame) {
    return pZeDMD->RenderGray2(frame);
}

ZEDMDAPI void
ZeDMD_RenderGray4(ZeDMD* pZeDMD, uint8_t* frame) {
    return pZeDMD->RenderGray4(frame);
}

ZEDMDAPI void
ZeDMD_RenderColoredGray6(ZeDMD* pZeDMD, uint8_t* frame, uint8_t* rotations) {
    return pZeDMD->RenderColoredGray6(frame, rotations);
}

ZEDMDAPI void
ZeDMD_RenderRgb24(ZeDMD* pZeDMD, uint8_t* frame) {
