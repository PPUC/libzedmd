...
bool ZeDMDWiFi::StreamBytes(ZeDMDFrame* pFrame) {
  ...
  if (pFrame->size < ZEDMD_COMM_FRAME_SIZE_COMMAND_LIMIT) {
    ...
    for (int i = 0; i < 3; i++) {
#if defined(_WIN32) || defined(_WIN64)
      sendto(m_wifiSocket, (const char*)data, pFrame->size + 4, 0,
             (struct sockaddr*)&m_wifiServer, sizeof(m_wifiServer));
#else
      sendto(m_wifiSocket, data, pFrame->size + 4, 0,
             (struct sockaddr*)&m_wifiServer, sizeof(m_wifiServer));
#endif
    ...
    data[0] = pFrame->command;  // command
    data[1] = (uint8_t)(128 | (pFrame->size / (m_zoneWidth * m_zoneHeight * 3 +
                                               1)));  // compressed + zone index

    mz_ulong compressedSize = mz_compressBound(pFrame->size);
    int status =
        mz_compress(&data[4], &compressedSize, pFrame->data, pFrame->size);
    data[2] = (uint8_t)(compressedSize >> 8 & 0xFF);
    data[3] = (uint8_t)(compressedSize & 0xFF);

    if (status == MZ_OK) {
#if defined(_WIN32) || defined(_WIN64)
      sendto(m_wifiSocket, (const char*)data, compressedSize + 4, 0,
             (struct sockaddr*)&m_wifiServer, sizeof(m_wifiServer));
#else
      sendto(m_wifiSocket, data, compressedSize + 4, 0,
             (struct sockaddr*)&m_wifiServer, sizeof(m_wifiServer));
#endif
      return true;
    }
