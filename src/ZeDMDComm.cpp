...
void ZeDMDComm::SetLogCallback(ZeDMD_LogCallback callback,
                               const void* userData) {
...
bool ZeDMDComm::Connect(char* pDevice) {
  ...
  sp_blocking_write(m_pSerialPort, (void*)CTRL_CHARS_HEADER,
                    CTRL_CHARS_HEADER_SIZE, ZEDMD_COMM_SERIAL_WRITE_TIMEOUT);
  sp_blocking_write(m_pSerialPort, (void*)data, 1,
                    ZEDMD_COMM_SERIAL_WRITE_TIMEOUT);

  std::this_thread::sleep_for(std::chrono::milliseconds(200));

  if (sp_blocking_read(m_pSerialPort, data, 8,
    ...
    if (!memcmp(data, CTRL_CHARS_HEADER, 4)) {
      ...
      if (sp_blocking_read(m_pSerialPort, data, 1,
                           ZEDMD_COMM_SERIAL_READ_TIMEOUT) &&
          data[0] == 'R') {
        ...
        if (sp_blocking_read(m_pSerialPort, data, 1,
                             ZEDMD_COMM_SERIAL_READ_TIMEOUT) &&
            data[0] == 'A' &&
            sp_blocking_read(m_pSerialPort, data, 1,
                             ZEDMD_COMM_SERIAL_READ_TIMEOUT) &&
            data[0] == 'R') {
          ...
          if (sp_blocking_read(m_pSerialPort, data, 1,
            ...
            if (sp_blocking_read(m_pSerialPort, data, 1,
              ...
              if (pDevice) {
                Log("ZeDMD found: device=%s, width=%d, height=%d", pDevice,
                    m_width, m_height);
              } else {
...
bool ZeDMDComm::StreamBytes(ZeDMDFrame* pFrame) {
  ...
  if (pFrame->size == 0) {
    ...
    memcpy(data, CTRL_CHARS_HEADER, CTRL_CHARS_HEADER_SIZE);
    data[CTRL_CHARS_HEADER_SIZE] = pFrame->command;
    mz_compress(data + CTRL_CHARS_HEADER_SIZE + 3, &compressedSize,
  ...
  do {
    ...
    sp_blocking_read(m_pSerialPort, &flowControlCounter, 1,
                     ZEDMD_COMM_SERIAL_READ_TIMEOUT);
  } while (flowControlCounter != 0 &&
           flowControlCounter != m_flowControlCounter);
  ...
  if (flowControlCounter == m_flowControlCounter) {
    ...
    while (position < size && success) {
      ...
      uint8_t response;
      do {
        sp_blocking_read(m_pSerialPort, &response, 1,
                         ZEDMD_COMM_SERIAL_READ_TIMEOUT);
